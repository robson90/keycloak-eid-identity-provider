...
public class EidSamlResponseHandlerUnitTest {
  ...
  void receiveSamlResponseParsesPersonalDataAndInitiatesIdentityAuthentication() {
    ...
    RootAuthenticationSessionModel rootAuthSessionModel =
    ...
    try {
      modelConfigMap.put(
          ...
          new String(
              readAllBytes(Paths.get(workDir + "/keys/01samlRequestSignaturePrivateKey.txt"))));
      modelConfigMap.put(
          "samlResponseDecryptionPublicKey",
          new String(
              readAllBytes(Paths.get(workDir + "/keys/02samlResponseDecryptionPublicKey.txt"))));
      modelConfigMap.put(
          "samlResponseDecryptionPrivateKey",
          new String(
              readAllBytes(Paths.get(workDir + "/keys/03samlResponseDecryptionPrivateKey.txt"))));
      modelConfigMap.put(
          "samlResponseVerificationCertificate",
          new String(
              readAllBytes(
                  Paths.get(workDir + "/keys/04samlResponseVerificationCertificate.txt"))));
      modelConfigMap.put(
          "samlRequestEncryptionCertificate",
          new String(
              readAllBytes(Paths.get(workDir + "/keys/05samlRequestEncryptionCertificate.txt"))));
    } catch (IOException e) {
      throw new RuntimeException(e);
    }

    SamlConfigurationImpl samlConfiguration = new SamlConfigurationImpl(modelConfigMap, "master");
    EidSamlResponseHandler sut =
        new EidSamlResponseHandler(
            realm,
            session,
            callback,
            event,
    ...
    assertEquals(
        "0f012961e06696c093b2f81412724e9556d115c57c6e4a15f1654b39196b7aab", identityValue.getId());
  }
}
